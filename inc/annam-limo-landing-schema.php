<?php
/**
 * JSON-LD cho landing Limousine HN–Sapa.
 *
 ...
 */
function annam_limo_landing_get_schema_graph() {
	if ( ! function_exists( 'annam_limo_landing_is_template' ) || ! annam_limo_landing_is_template() ) {
		return null;
	}
	...
	$offers = array();
	if ( ! empty( $config['pricing']['rows'] ) && is_array( $config['pricing']['rows'] ) ) {
	...
	$graph[] = array(
		...
		'itemListElement' => array(
			...
			array(
				'@type'    => 'ListItem',
				'position' => 2,
				'name'     => isset( $config['hero']['title'] ) ? wp_strip_all_tags( (string) $config['hero']['title'] ) : 'Limousine HN–Sapa',
				'item'     => $page_url,
			),
		),
	);
	...
	return apply_filters(
		'annam_limo_landing_schema_graph',
		array(
...
function annam_limo_landing_print_schema() {
	if ( ! apply_filters( 'annam_limo_landing_schema_print', true ) ) {
		return;
	}
	$data = annam_limo_landing_get_schema_graph();
	if ( empty( $data['@graph'] ) ) {
		return;
	}
...
add_action( 'wp_head', 'annam_limo_landing_print_schema', 8 );
